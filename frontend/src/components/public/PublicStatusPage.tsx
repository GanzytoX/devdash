...
export function PublicStatusPage() {
  ...
  return (
    <main className="min-h-screen bg-[#040815] text-slate-100 px-3 sm:px-4 py-6 sm:py-10">
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div><p className="text-brand-blue-400 font-mono text-[10px] sm:text-xs uppercase tracking-widest">Estado público de DevDash</p><h1 className="text-2xl sm:text-3xl font-bold mt-1 break-words">{data?.instance || 'Estado de los servicios'}</h1><p className="text-slate-400 text-sm mt-1">{data?.region || 'Infraestructura de CubePath'}</p></div>
          <a href="/" className="glass-btn px-4 py-2.5 text-xs inline-flex items-center justify-center gap-2 w-full sm:w-auto">Panel del operador <ExternalLink className="h-3.5 w-3.5" /></a>
        </header>
        ...
        {data && <>
          <section className={`rounded-2xl p-4 sm:p-6 border flex items-start sm:items-center gap-3 sm:gap-4 ${
            !hasServices
            ...
            {!hasServices || allServicesPaused
              ? <Server className="h-7 w-7 sm:h-8 sm:w-8 text-slate-400 shrink-0" />
              : data.operational
                ? <CheckCircle2 className="h-7 w-7 sm:h-8 sm:w-8 text-emerald-400 shrink-0" />
                : <TriangleAlert className="h-7 w-7 sm:h-8 sm:w-8 text-amber-400 shrink-0" />}
          ...
          <section className="space-y-3" aria-labelledby="services-title"><h2 id="services-title" className="font-bold">Servicios monitoreados</h2>
            ...
            ) : data.services.map(service => <article key={service.id} className="glass-panel rounded-2xl p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-semibold break-words">{service.name}</h3><div className="flex flex-wrap gap-1 mt-2">{service.tags.map(tag => <span key={tag} className="text-[9px] font-mono bg-white/5 px-2 py-1 rounded break-all">{tag}</span>)}</div></div><span className={`text-[10px] sm:text-xs font-bold uppercase shrink-0 ${service.status === 'online' ? 'text-emerald-400' : service.status === 'degraded' ? 'text-amber-400' : service.status === 'paused' || service.status === 'unknown' ? 'text-slate-400' : 'text-red-400'}`}>{statusLabel(service.status)}</span></div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5 text-sm"><div><p className="text-[10px] text-slate-500 uppercase">Disponibilidad</p><p className="font-mono mt-1">{service.uptime === null ? '—' : `${service.uptime}%`}</p></div><div><p className="text-[10px] text-slate-500 uppercase">Latencia promedio</p><p className="font-mono mt-1">{service.averageLatency === null ? '—' : `${service.averageLatency} ms`}</p></div><div><p className="text-[10px] text-slate-500 uppercase">SSL</p><p className="font-mono mt-1">{sslLabel(service.sslStatus)}</p></div></div>
          ...
          <section className="glass-panel rounded-2xl p-4 sm:p-5"><h2 className="font-bold flex items-center gap-2"><Clock3 className="h-4 w-4" /> Incidentes recientes</h2><div className="mt-4 divide-y divide-white/5">{data.services.length === 0 ? <p className="text-sm text-slate-400 py-4">No hay servicios públicos para consultar incidentes.</p> : data.incidents.length === 0 ? <p className="text-sm text-slate-400 py-4">No hubo incidentes durante este periodo.</p> : data.incidents.map(i => <div key={i.id} className="py-3 flex flex-col sm:flex-row sm:justify-between gap-2 sm:gap-4 text-sm"><div className="min-w-0"><p className="font-medium">{i.service.name}</p><p className="text-slate-400 text-xs mt-1 break-words">{i.message}</p></div><span className={`shrink-0 ${i.status === 'resolved' ? 'text-emerald-400' : 'text-red-400'}`}>{statusLabel(i.status)}</span></div>)}</div></section>
        </>}
        <footer className="text-center text-xs text-slate-500 pt-4 flex flex-wrap justify-center items-center gap-2"><Server className="h-3.5 w-3.5" /> Alojado en <a className="text-brand-blue-400" href="https://cubepath.com" target="_blank" rel="noopener noreferrer">CubePath</a><span>·</span><ShieldCheck className="h-3.5 w-3.5" /> Desarrollado con DevDash <Activity className="h-3.5 w-3.5" /></footer>
      </div>
    </main>
  );
}
