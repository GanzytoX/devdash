  ...
  return (
    <main className="min-h-screen bg-[#040815] text-slate-100 px-4 py-10">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {data && <>
          <section className={`rounded-2xl p-6 border flex items-center gap-4 ${data.operational ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}>
            {data.operational ? <CheckCircle2 className="h-8 w-8 text-emerald-400" /> : <TriangleAlert className="h-8 w-8 text-amber-400" />}
            <div><h2 className="font-bold text-lg">{data.operational ? 'Todos los sistemas funcionan correctamente' : 'Algunos sistemas presentan problemas'}</h2><p className="text-xs text-slate-400 mt-1">Actualizado el {new Date(data.updatedAt).toLocaleString()}</p></div>
          </section>
          ...
          <section className="space-y-3" aria-labelledby="services-title"><h2 id="services-title" className="font-bold">Servicios monitoreados</h2>
            {data.services.map(service => <article key={service.id} className="glass-panel rounded-2xl p-5">
              <div className="flex items-start justify-between gap-4"><div><h3 className="font-semibold">{service.name}</h3><div className="flex gap-1 mt-2">{service.tags.map(tag => <span key={tag} className="text-[9px] font-mono bg-white/5 px-2 py-1 rounded">{tag}</span>)}</div></div><span className={`text-xs font-bold uppercase ${service.status === 'online' ? 'text-emerald-400' : service.status === 'paused' ? 'text-slate-400' : 'text-red-400'}`}>{statusLabel(service.status)}</span></div>
